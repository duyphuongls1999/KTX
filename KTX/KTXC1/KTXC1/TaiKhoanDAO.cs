...
namespace KTXC1
{
    public class TaiKhoanDAO
    {
        ...
        {
            string sql = @"SELECT COUNT(*) FROM TAIKHOAN WHERE tenDangNhap=@tendangnhap and matKhau = @mk ";
        ...
        public TaiKhoan1 LayNhanVien(string manv)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string sql1 = @"SELECT * FROM TAIKHOAN WHERE tenDangNhap = @tendangnhap,maNV = @manv";
        ...
        public DataTable LayNhanVien()
        {
            ...
            string sql = @"SELECT * FROM NHANVIEN,TAIKHOAN where NHANVIEN.maNV = TAIKHOAN.maNV";
            SqlCommand command = new SqlCommand(sql, connection);
        ...
        public bool Them(TaiKhoan1 nv)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string sql1 = @"INSERT INTO TAIKHOAN(tenDangNhap,matKhau,maNV) VALUES(@tendangnhap, @matkhau,@manv)";
                {
                    SqlCommand command = new SqlCommand(sql, connection);
        ...
        public bool ChinhSua(TaiKhoan1 nv)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string sql1 = @"UPDATE TAIKHOAN SET tenDangNhap= @tendangnhap, matKhau= @matkhau WHERE maNV = @manv";
                SqlCommand command = new SqlCommand(sql, connection);
                SqlCommand command1 = new SqlCommand(sql1, connection);
                command.Parameters.AddWithValue("@manv", nv.MaNV);
                command.Parameters.AddWithValue("@ten", nv.TenNV);
        ...
        public bool Xoa(string manv)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string sql1 = @"DELETE FROM TAIKHOAN WHERE tenDangNhap = @tendangnhap";
                SqlCommand command = new SqlCommand(sql, connection);
                SqlCommand command1 = new SqlCommand(sql1, connection);
                command.Parameters.AddWithValue("@manv", manv);
